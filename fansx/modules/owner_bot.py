...
async def _(client, message):
    ...
    if not user_id:
        return await msg.edit(f"<b>{message.text} ᴜsᴇʀ_ɪᴅ/ᴜsᴇʀɴᴀᴍᴇ - ʙᴜʟᴀɴ</b>", parse_mode=ParseMode.HTML)
    ...
    if user.id in prem_users:
        return await msg.edit(
            f"""
<blockquote><b>ɴᴀᴍᴇ:</b> [{user.first_name} {user.last_name or ''}](tg://user?id={user.id})<br>
<b>ɪᴅ:</b> <code>{user.id}</code><br>
<b>ᴋᴇᴛᴇʀᴀɴɢᴀɴ:</b> ꜱᴜᴅᴀʜ ᴘʀᴇᴍɪᴜᴍ<br>
<b>ᴇxᴘɪʀᴇᴅ:</b> {get_bulan} ʙᴜʟᴀɴ</blockquote>
""",
...
""",
        ...
        return await bot.send_message(
            OWNER_ID,
            f"• ɪᴅ-ꜱᴇʟʟᴇʀ: <code>{message.from_user.id}</code>\n\n• ɪᴅ-ᴄᴜꜱᴛᴏᴍᴇʀ: <code>{user_id}</code>",
            parse_mode=ParseMode.HTML,
        ...
        return await msg.edit(str(error), parse_mode=ParseMode.HTML)
...
name: [{user.first_name} {user.last_name or ''}](tg://user?id={user.id})
id: {user.id}
keterangan: sudah dalam daftar owner
"""
        )
